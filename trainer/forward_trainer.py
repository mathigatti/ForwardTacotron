...
class ForwardTrainer:
    ...
    def generate_plots(self, model: Union[ForwardTacotron, FastPitch], session: TTSSession) -> None:
        ...
        gen = model.generate(batch['x'][0:1, :batch['x_len'][0]], model.de_semb.unsqueeze(0).to(device))
        ...
        gen_en = model.generate(batch['x'][0:1, :batch['x_len'][0]], model.en_semb.unsqueeze(0).to(device))
        m2_hat = np_now(gen_en['mel_post'].squeeze())
        m2_hat_fig = plot_mel(m2_hat)
        self.writer.add_figure('EN_Generated/postnet', m2_hat_fig, model.step)
        m2_hat_wav = self.dsp.griffinlim(m2_hat)
        self.writer.add_audio(
            tag='EN_Generated/postnet_wav', snd_tensor=m2_hat_wav,
            global_step=model.step, sample_rate=self.dsp.sample_rate)